...
def bin_op_handler(bin_op_str):
    return parse_variables(bin_op_str)


def extract_before_p(j,str):
    i = j
    while str[i] != "(":
        i+=1
    return str[j:i-1]

def unary_op_handler(unary_op_str):
    i = unary_op_str.find("operand=")
    tmp = extract_before_p(i + len("operand="), unary_op_str)
    tmp_str = ""
    if tmp == "BinOp":
        tmp_str = parse_parenthesis(i + len("operand=BinOp")+1,unary_op_str)
        bin_op_handler(tmp_str)
    elif tmp == "Name":
        tmp_str = parse_parenthesis(i + len("operand=Name") + 1, unary_op_str)
    elif tmp == "BinOp":
        tmp_str = parse_parenthesis(i + len("operand=UnaryOp")+1,unary_op_str)
        unary_op_handler(tmp_str)
    return parse_variables(tmp_str)
...
def track_while(while_str, while_list):
...
def if_branch(parent_list,global_while_list, called_by_fun, fun_global, if_ladder,leaf_str):

    if "orelse=" not in if_ladder:
        # print "termination",if_str
        if if_ladder[0:2] == "[]":  # absence of else part
            return []
        else:  # handling else part
            else_str = if_ladder
            continuous_parse(parent_list[:], global_while_list, called_by_fun, fun_global, else_str)
    ...
    tmp = split_through_orelse(if_ladder)
    if_half = tmp[0]
    ladder = tmp[1]

    if if_ladder[1:5] != "test":
    ...
    """then extract body part and process like normal AST text """
    i = if_half[i:].find("body=[")
    body_str = parse_square_br(i+len("body="),if_half)[0]

    # process body part
    continuous_parse(parent_list[:], global_while_list, called_by_fun, fun_global, body_str)

    # process leaf_str
    continuous_parse(parent_list[:], global_while_list, called_by_fun, fun_global, leaf_str)

    #process ladder part
    if_branch(parent_list, global_while_list, called_by_fun, fun_global, ladder, leaf_str)
...
def continuous_parse(parent_list, global_while_list, called_by_fun, fun_global, data):
    ...
    while i < length - 1:
        ...
        if parse_keyword(i, data) == "Assign":
            ...
            i = tmp[1]
            if nested_while == 0:
                parent_list += track_while(while_str,[])
            nested_while += 1
