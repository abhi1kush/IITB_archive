...
def set_clear_denning(G, called_by_fun, fun_global, expr_str, PC):
    ...
    if attr == "set" or attr == "clear":
        ...
        # treat it like AugAssign s0 -= 1
        pc_update(PC,convertToLabelList([var_name]))
        #print "set clear-> ",PC
        for pc in PC:
            print make_lub_string(pc)+ " " + const.lt + " " + var_name #label[left]
    elif attr == "wait":
...
def duplicateRemoval(PC):
    ...
    for pc in PC:
        ...
        tmpPC.append(tmppc)
    tmpPC.sort()
    #print "dup",PC,list(tmpPC for tmpPC,_ in itertools.groupby(tmpPC))
...
def continuous_parse(G, funPC, called_by_fun, fun_global, data, PC):
    ...
    while i < length - 1:
        ...
        if parse_keyword(i,data) == "FunctionDef":  #skipping all function definition
            ...
            #print i+1,tmp[1],len(tmp[0]),"function",tmp[0]
            i = tmp[1]
        ...
        if parse_keyword(i, data) == "set_clear_wait":
            ...
            if expr_str.find("'set'") != -1 or expr_str.find("'clear'") != -1 or expr_str.find("'wait'") != -1 :
                i = tmp[1]
                set_clear_denning(G, called_by_fun, fun_global, expr_str, PC)
        if parse_keyword(i, data) == "AugAssign":
            i += 9
            tmp = parse_parenthesis(i, data)
            augAssign_str = tmp[0]
