...
def parse_keyword(i, data):
    # checking for
    if i +  len("Expr(value=Call(func=Name(id='") < len(data) and data[i:i + len("Expr(value=Call(func=Name(id='")] == "Expr(value=Call(func=Name(id='":
	return  "Expr(value=Call(func=Name(id='"
...
def label_to_lub(llist):
    print label,"label_to_lub @@@@@@"
    if len(llist) == 0:
        return "Low"
    if len(llist) == 1:
        return llist[0]
    ...
    i = 1
    while i<len(llist):
        ret += " "+const.oplus+" "+ llist[i]
        i +=1
    print label,"label_to_lub END @@@@@@"
    return ret


def make_lub_string_label(llist):
    print label,"make_lub_string_label @@@@@@"
    if len(llist) == 0:
        return "Low"
    if len(llist) == 1:
        return label_to_lub(label[llist[0]])
    tmp = set(llist)
    uniq_list = list(tmp)
    if len(uniq_list) == 1:
        return label_to_lub(label[uniq_list[0]])
    ret = ""
    ret += label_to_lub(label[uniq_list[0]])
    i = 1
    while i < len(uniq_list):
        ret += " " + const.oplus + " "
        ret += label_to_lub(label[uniq_list[i]])
        i += 1
    print label,"make_lub_string_label END @@@@@@"
    return ret

def convertToLabelList(llist):
    if len(llist) == 0:
        return llist
    ret = []
    for it in llist:
...
def assign_denning(G,assign_str, PC):  # applying dennig's model on assignments
    #pdb.set_trace()
    global line
    ...
    if left in G:
        for pc in PC:
            print make_lub_string(pc)+ " " + const.lt + " " + left #label[left]
    else:
        # update dynamic label, for all x (MNG(BR) union {pc}) [lambda(x)=l+lambda(x)+lambda(pc)]
        label[left] = rvalue
        #print label[left],"changed", left
    line += 1
    return PC[:]
...
def augAssign_denning(called_by_fun, fun_global, augAssign_str, PC):
    ...
    pc_update(PC,Sr)
    for pc in PC:
        print make_lub_string(pc) + " " + const.lt + " " + Sr[0]
    return PC[:]
...
def if_denning(G,called_by_fun, fun_global, if_str, rest, PC):
    ...
    if "orelse=" not in if_str:
        ...
        if if_str[0:2] == "[]":  # absence of else part
            ...
            else_str = if_str
            continuous_parse(G,[], ccalled_by_fun, fun_global, else_str, PC[:])
            return []
    ...
    i = tmp[1]
    pc_update(PC,convertToLabelList(parse_variables(test_str)))
    ...
    PCA = list(continuous_parse(G,[], called_by_fun, fun_global, body_str , PC1[:]))
    PCB = list(continuous_parse(G,[], called_by_fun, fun_global, ladder , PC2[:]))
    #debugPrint(PCA)
    #debugPrint(PCB)
    return PCB + PCA
...
def while_denning(iteration, G, called_by_fun, fun_global, while_str, PC):
    ...
    if while_str[6:10] == "Name":
        ...
        compare = tmp[0]
        i = tmp[1]
    pc_update(PC,convertToLabelList(parse_variables(compare)))
    # body processing
    ...
    PCB = list(continuous_parse(G, [], called_by_fun, fun_global, body_str, PC2[:]))
    return PC1 + PCB



def set_clear_denning( global_while_list, expr_str):
...
def fun_denning(fun_str,PC):
    ...
    funPC = []
    PC = continuous_parse(G,funPC, fun_name, fun_globals, fun_str,PC) 
    return PC + funPC
...
recCount =0
def continuous_parse(G, funPC, called_by_fun, fun_global, data, PC):
    #pdb.set_trace()
    ...
    while i < length - 1:
        # checking for keyword
        if parse_keyword(i,data) == "FunctionDef":
            i = parse_parenthesis(i+11, data)[1]
            print data[i]
        if parse_keyword(i,data) == "Expr(value=Call(func=Name(id='":
            ...
            i = parse_parenthesis(i+5, data)[1]
        ...
        if parse_keyword(i, data) == "Assign":
            ...
            if "value=Name(id='threading'" in assign_str:
                continue
            tmp = assign_denning(G,assign_str, PC[:])
            memo = {}
            ls = uniq(tmp)
            PC = copy._deepcopy_list(ls,memo)
        elif parse_keyword(i, data) == "If":
            ...
            i = tmp[1]
            rest = tmp[2]
            tmp = if_denning(G,called_by_fun, fun_global, if_str, rest, PC[:])
            ls = uniq(tmp)
            memo = {}
            PC = copy._deepcopy_list(ls, memo)
        elif parse_keyword(i, data) == "While":
            ...
            while( it <= iteration ):
                ...
                tmp = while_denning(it, G, called_by_fun, fun_global, while_str, PC[:])
                ls = uniq(tmp)
                memo = {}
                PC = copy._deepcopy_list(ls, memo)
                if lastPC == PC:
...
def process_label_fille(filename):
    # a = x,y,z
    label = {}
...
PC = [[]]
label = {}
label = process_label_fille(sys.argv[2])
gg = []
for key in label:
    gg.append(key)
G = set(gg)
...
#G = Global(data)
continuous_parse(G,"", [], dummy, data, PC)
